...
This project runs in simulation mode or on a roboRIO v1 for all three vision systems. The only requirements to run this minimal
example in simulation mode are a DriverStation PC and camera. The WPILib AprilTag and pose calculations are included or the free
of charge PhotonVision may be downloaded and simply run on the PC. A real roboRIO v1 may be used for running on a real robot.
...
A purchased LimelightVision box may be used. A purchased co-processor for PhotonVision may be used.

For competitive use of the WPILib vision, a roboRIO v1 may be used only for turn to an angle targeting (3-D but no time left for
other processes). A roboRIO v2 is required for competitive use of the WPILib 3-D pose. A low cost, high quality recommendation is
to use PhotonVision on an inexpensive, recommended coprocessor.