 ...
 * <p>
 * In ReefScape 2025 the reef scoring positions pairs (left/right) were identically relative
 * to their designated tags. Validation is done to assure the best tag in view is one of those in
 * the set of identically relative targets.
 * <p>
 ...
 * tag and two scoring positions - nothing fancy for validation and transform determination.
 * <p>
 * Related references:
...
public class AlignToReefTargetRelativeTransform3D extends Command {
    ...
    private List<Integer> tagIDsDesired = new ArrayList<>();
    ...
    public AlignToReefTargetRelativeTransform3D(boolean isRightScore, VisionContainer visionContainer,
        ...
        // Each of these reef tags identically mark two nearby scoring positions
        tagIDsDesired.add(6);
        tagIDsDesired.add(7);
        tagIDsDesired.add(8);
        tagIDsDesired.add(9);
        tagIDsDesired.add(10);
        tagIDsDesired.add(11);
        tagIDsDesired.add(17);
        tagIDsDesired.add(18);
        tagIDsDesired.add(19);
        tagIDsDesired.add(20);
        tagIDsDesired.add(21);
        tagIDsDesired.add(22);
 
        // The robot can score at a position to the right or left of the reef AprilTags
        if (isRightScore) { // right
    ...
    public void execute() {
        ...
        if (!tagIDsDesired.contains(pose.AprilTagId)) // make sure still looking at a correct tag
        {
            System.out.println("Oops! " + pose.AprilTagId + " is not a reef tag ");
            return;
        }
