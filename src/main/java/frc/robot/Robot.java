...
/**
 * <p>This Project utilizes vision systems to detect and decode AprilTags in order to compute the robot position.
 * 
 * <p>Three example uses of Vision using different targeting strategies are included as commands:
 ...
 * <P> 3. Drive the robot to a specified position relative to the target (Transform3d). This is similar to example #2 but instead of
 * using field coordinates it uses a displacement from a target. {@link AlignToReefTargetRelativeTransform3D}
 * (activated by XBox controller X button {@link RobotContainer#configXButton()})
 *
 ...
 * 
 * <p>This project is a wrapper called {@link VisionContainer} for three lower level wrappers {@link ControllerVision}
 * {@link PhotonVision} {@link LimelightVision} of three complex vision systems - WPILib, PhotonVision, and LimelightVision.
 * 
 * After selecting and configuring the desired vision system as described below, use the robot pose as in an example commands
 * {@link AlignToReefFieldRelativePose3D} or {@link AlignToReefTagRelativeArcade2D} or {@link AlignToReefTargetRelativeTransform3D}
 * 
 * <p>See {@link VisionContainer} as an example of code to instantiate each of the three vision systems and use the RobotPose
 * provided by each of the vision classes.
 * 
 ...
 * tags in view in a frame. That is likely wrong a lot of the time. Some smarts need to be added so targeting goes to
 * the right target; see {@link ControllerVision#getPose3d()} and below it.
 * 
 ...
 * A commented dummy stub is included to give a starting hint of what could be done but wasn't in this project. Otherwise, the simpler
 * "bestTarget" as used in this project is a good approximation.
 * <p>Use of the WPILib PoseEstimator requires that values from this vision targeting project be merged with the gyro
 ...
 * <p>PhotonVision running on some device - the simulation PC or a coprocessor attached to the same network as the simulation PC
 *
 * <p>This code has been tested with Limelight 2+ limelight2_2025_1_release, PhotonVision v2026.0.1-beta on Raspberry Pi 4 Model B Rev 1.2,
 *  PhotonVision v2026.0.1-beta on Windows 11, and WPILib 2025.3.2; all with both a roboRIO v1 and DriverStation PC simulation.
 ...
 * <p>Gyro was simulated with a "0" heading and "0" rate.
 * <p>Drivetrain was crudely simulated by printing the commanded velocity and voltage values and PWM motor controllers.
 * 
 * <p>Note that a significant amount of logging is turned on including @Logged annotation, datalog to a file and terminal output. These are
 * examples and not required for correct vision operation. There are some helpful examples of debugging prints in the code most but not all
 * of which are commented out. Command execution summary logging and spike removal are printed as are fake drivetrain data. Pose alignment
 * discrepancies are printed so the human can pretend to drive (the camera) to the target.
 * 
 * <p>--------------------------------------------------------
 * <p>-----Setting up simulation of the roboRIO on the PC-----
 * <p>Get the ip address of the PC running the simulation by command prompt program "ipconfig" (linux command is "ifconfig"). If the IP
 * address isn't correct initially causing a connection issue in simulation or real, no harm is done and this program provides some
 * guidance to correctly enter the team number and lists the IP address on the terminal.
 * 
 * <p>Optional for the curious: Start Outline Viewer on the PC and change Options/settings/team/IP 127.0.0.1 (or localhost or the PC IP address) - client nt4 mode and apply
 * <p>For PhotonVision start the device/program and change Settings Team Number/NetworkTables Server Address to the PC IP address PV needs to see the Server
 * <p>For LimelightVision start the device and change Settings Custom NT Server IP: to the PC IP address LL needs to see the Server
 ...
 * <p>If desired, unneeded vision code may be stripped out based on each file's use:
 * <p>  AcquireAprilTag.java                      CV
 * <p>  AcquireRobotPose.java                     CV
 * <p>  AlignToReefFieldRelativePose3D.java       CV, PV, LV (neutral example command)
 * <p>  AlignToReefTargetRelativeTransform3D.java CV, PV, LV (neutral example command)
 * <p>  AlignToReefTagRelativeArcade2D.java       CV, PV, LV (neutral example command)
 * <p>  AprilTagsLocations.java                   CV, PV, LV (LV only by association with the example alignment commands that may use the tag locations)
 * <p>  CameraBase.java                           CV, PV, LV
 * <p>  CommandSchedulerLog.java                  CV, PV, LV
 * <p>  ControllerVision.java                     CV
 * <p>  Drivetrain.java                           CV, PV, LV (fake for drivetrain and gyro for alignment commands)
 * <p>  Image.java                                CV
 * <p>  LimelightHelpers.java                             LV
 * <p>  LimelightVision.java                              LV
 * <p>  Main.java                                 CV, PV, LV
 * <p>  PhotonVision.java                             PV                       
 * <p>  Robot.java                                CV, PV, LV
 * <p>  RobotContainer.java                       CV, PV, LV
 * <p>  RobotPose.java                            CV, PV, LV
 * <p>  SpikeFilter.java                          CV
 * <p>  VisionContainer.java                      CV, PV, LV (four cases of the two undesired vision processes may be removed)
 * 
