...
public class AcquireRobotPose {
  ...
  AprilTagsLocations aprilTagsLocations;
  ...
  public AcquireRobotPose(ControllerVision roboRIOCamera, boolean usePose3D, Transform3d cameraInRobotFrame /*robotToCamera*/)
  {
    ...
    this.roboRIOCamera = roboRIOCamera;
  ...
  /**
   * Acquire Robot Pose from detected AprilTag processing loop
   */
  public void run() {
      ...
      for (AprilTagDetection detection : detections) {
        ...
        {
          System.out.println("bad decision margin for id " + detection.getId() + ", " + detection.getDecisionMargin());
          continue;
        }
        ...
        SmartDashboard.putNumber("tag " + detection.getId() + " x (yaw) angle [deg]", yaw);
        ...
        { // draw a frustum in front of the AprilTag
          ...
          float[] cameraParm = {(float)roboRIOCamera.cameraFx,   0.f,             (float)roboRIOCamera.cameraCx,
                                  0.f,             (float)roboRIOCamera.cameraFy, (float)roboRIOCamera.cameraCy,
                                  0.f,             0.f,             1.f};
          ...
          */
          ...
          Calib3d.projectPoints(bottom, R, T, K, roboRIOCamera.distortionCoeffs, imagePointsBottom);

          MatOfPoint2f imagePointsTop = new MatOfPoint2f();
          Calib3d.projectPoints(top, R, T, K, roboRIOCamera.distortionCoeffs, imagePointsTop);
      ...
      synchronized(this) { // lock the list while it's being completely redone with the accumulated new data
      ...
      SmartDashboard.putNumber("theoretical fps (1/frame time)", 1./(endFrameTime - acquisitionTime.acquisitionTime));
    }
