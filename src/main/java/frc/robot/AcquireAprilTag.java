...
public class AcquireAprilTag {
  ...
  static
  {
    ...
    AcquireAprilTag(ControllerVision roboRIOCamera)
    {
        ...
        UsbCamera camera = CameraServer.startAutomaticCapture(roboRIOCamera.cameraDeviceId); // http://10.42.37.2:1181/   http://roborio-4237-frc.local:1181/?action=stream
              //"myCam", "/dev/v4l/by-id/usb-Arducam_Technology_Co.__Ltd._Arducam_OV9281_USB_Camera_UC762-video-index0"
        camera.setResolution(roboRIOCamera.cameraW, roboRIOCamera.cameraH);
        camera.setFPS(roboRIOCamera.fps);
  ...
  /**
   * Acquire a camera frame. Intended to be iterated in an external loop.
   */
  public void run() {
      ...
      roboRIOCamera.image.setImage(mat, detections, acquisitionTime, frameNumber);
    }
}
