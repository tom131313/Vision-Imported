 ...
 * 
 * <p>cameraToTarget Transform3d orientation:
 * <p>x+ is tag is forward of the camera, y+ is tag is left of the camera, z+ is tag is up from the camera
 * 
 * <p>If the camera and tag are straight-on then yaw is +180 deg or -180 deg since the tag must be pointing at
 * the camera to be seen (facing each other).
 * 
 * <p>with tag facing camera:
 * <p>+roll is tag rotated ccw wrt the camera
 * <p>+pitch is top of the tag moved toward the camera
 * <p>+yaw  straight-on -180, -179 ... -> 0 as tag rotates around to the left;
...
public class RobotPose {
    ...
    public String toString() {
        return String.format("tag %d, yaw [deg] %f, pitch [deg] %f, %s, %s", AprilTagId, yaw, pitch, pose3D, cameraToTarget);
    }
}
